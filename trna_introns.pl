...
open(trnabed,"<", "../trna_mappings/hg38-tRNAs.bed")or die("unable to open trnabed");
my %introns;
while(my $line=<trnabed>)
{
    ...
    if($#blocks>0)
    {
        if($array[5]eq"+")
        {
        $introns{"hg38_tRNAs_".$array[3]}=[$blocks[0]+1, $starts[1]-1];
        }
        elsif($array[5]eq"-")
        {
        $introns{"hg38_tRNAs_".$array[3]}=[$blocks[1]+1,$blocks[1]+$starts[1]-$blocks[0]-1];    
        }
    }
...
open(trnaseq,"<", "../trna_mappings/trna_sequences.fa")or die("unable to open trnaseq");
my %seq;
my $trna;
...
open(sam_AGO1,"<", "../trna_mappings/AGO1_mappings_trna_sort.sam")or die("unable to open sam_AGO1");
my @AGO1;
my %AGO_hash;
...
open(sam_AGO2,"<", "../trna_mappings/AGO2_mappings_trna_sort.sam")or die("unable to open sam_AGO2");
my @AGO2;
...
open(sam_AGO3,"<", "../trna_mappings/AGO3_mappings_trna_sort.sam")or die("unable to open sam_AGO3");
my @AGO3;
...
open(sam_AGO4,"<", "../trna_mappings/AGO4_mappings_trna_sort.sam")or die("unable to open sam_AGO4");
my @AGO4;
...
open(sam_Dicer1,"<", "../trna_mappings/Dicer_rep1_mappings_trna_sort_rmdup.sam")or die("unable to open sam_Dicer1");
my @Dicer1;
my %Dicer_hash;
...
open(sam_Dicer2,"<", "../trna_mappings/Dicer_rep2_mappings_trna_sort_rmdup.sam")or die("unable to open sam_Dicer2");
my @Dicer2;
...
open(sam_Dicer3,"<", "../trna_mappings/Dicer_rep3_mappings_trna_sort_rmdup.sam")or die("unable to open sam_Dicer3");
my @Dicer3;
...
#}
open(file,">", "../trna_mappings/AGO_Dicer_trna.fasta")or die("unable to open file");

foreach $key (keys %ago_num)
{
    if(exists($dicer_num{$key}))
    {
    print $key."\n";
    my @array=split(/=/,$key);
